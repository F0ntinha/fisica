document.addEventListener("DOMContentLoaded", () => {
  ...
  function iniciar() {
    ...
    function loop() {
      ...
      grafico.data.datasets[2].data.push(v*v / r); // aceleração centrípeta
      ...
      // Atualiza posição angular
      theta += v / r * 0.016; // Δθ = v/r * Δt
      tempo += 0.016;
  ...
  function desenharPista(r, ang) {
    ...
    ctx.rotate(-ang); // inclinação da pista
  ...
  function desenharCarro(r, theta, ang, v, vMax) {
    ...
    // Velocidade instantânea acima do carro
    ctx.fillStyle = "white";
    ctx.font = "14px Arial";
  ...
  function desenharVetores(m, v, r, theta, ang) {
    ...
    ctx.beginPath();
    ctx.moveTo(0,0);
    ctx.lineTo(Fc/50, 0); // vetor centrípeta
